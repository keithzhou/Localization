...
Kalman filter use observed variables to infer hidden variables and use that to help predict the next state. In our system, observed variables are the $x$ and $y$ coordinates of the system, and we also model unobserved motion variables velocity $v_x, v_y$ and acceleration variables $a_x, a_y$. Then the state variables we are tracking ca be represented as:
\[
\left[\begin{array}{c}
x\\
y\\
