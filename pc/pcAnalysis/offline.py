...
CV = {
  'no_phase,np_bpf': {'grid_resolution': 800,'doPhaseTransform':False, 'doBandpassFiltering':False},
...
for key in CV:
  ...
  for f in os.listdir(PATH):
    ...
    for data,freq in zip(d[0],d[1]):
      ...
      #print r,theta
    print "\tfile:",f,"r: %.2f theta: %.2f" %(r_gold,theta_gold),"error: %.4f" %np.mean(abs(np.array(rs) - r))
